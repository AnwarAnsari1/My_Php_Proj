...
export class OPBillingComponent implements OnInit, OnDestroy {
  ...
  formGroup: FormGroup;
  temp = { dept: '', category: '', feesId: 0, testType: '', feesName: '',qty:'' };
  ...
  @ViewChild('qty') qty: ElementRef<ViewChild>;
          ...
          this.service.getFees(method, id)
            ...
            .subscribe({
              next: response => {
                ...
                this.renderer.selectRootElement(this.qty.nativeElement).focus();
              }
            });
        }
      }
  ...
  addQty(evt: KeyboardEvent): void {
    if (evt.key === 'Enter' && this.formGroup.get('cost').value !== null && this.formGroup.get('qty').value !== null && this.tempData.dept !== '') {
      (this.formGroup.get('addedFees') as FormArray).push(
        this.fb.group({
          dept: [this.tempData.dept],
          category: [this.tempData.category],
  ...
  getDate() {
    return formatDate(new Date, 'yyyy-MM-dd', 'en-IN', 'IST');
  }
        ...
        .subscribe({
          next: (response) => {
            this.formGroup.patchValue({
              ...
              sampledate:response.data.collection_date
            });
  ...
  private initProperties() {
    ...
    this.formGroup = new FormGroup({
      ...
      sampledate:new FormControl(this.getDate()),
